...
description: "Code review through the lens of real engineers' documented philosophies (Torvalds, Thompson, Carmack, Kent Beck, Jobs, Cagan). Use when a PR or diff has architectural or UX impact, or you want pre-submit hardening beyond standard checks."
---
...
For whole-project or whole-tool keep/kill decisions, use the `adversarial-review` skill instead — this one is for reviewing a specific PR or diff.
...
2. **Attach a confidence level to every attribution** — `high` (documented, in `references/persona_principles.md` with a source), `moderate` (widely attributed, source not pinned), `low`/`unknown` (you're inferring).
...
Merge duplicates; count concurrences; promote per the rule; flag single-lens findings (often the most interesting).
...
- Generic review findings feel generic and you want sourced specificity.
- Reviewing methodologies or docs (product personas excel here).
- Auth, data, architecture, or public-API changes.

## When NOT to Use

- Low-impact PR (cosmetic only, no logic change).
...
- Evaluating whether a whole project/tool should exist — use `adversarial-review` instead.
...
- **Related, different job:** `adversarial-review` (personal skill) — whole-project/tool keep-kill-reshape-invest decisions, not diff-level review
...
- **Origin:** adapted from [alirezarezvani/claude-skills](https://github.com/alirezarezvani/claude-skills/tree/main/engineering-team/skills/named-persona-adversarial-review) (concept by @YuhaoLin2005, PR #866). Dropped references to sibling skills in that repo (`adversarial-reviewer`, `code-reviewer`, `senior-security`, `productivity/andreessen`) not installed here.
