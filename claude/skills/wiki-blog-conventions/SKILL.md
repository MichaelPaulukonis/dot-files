...
**⚠️ Moving a page: don't create+delete.** Recreating the page at a new path and deleting the old one **destroys its edit history** — already happened to `dragline` and `ia-book-page-downloader` this way, unrecoverable. Use the `wikijs` CLI skill's `move` command instead (`wikijs.py move {ref} {destination}`, wraps `pages.move`, preserves revision history). The `wikijs` MCP server (which had no move/rename tool) has been removed entirely — the CLI script is now the only interface.
